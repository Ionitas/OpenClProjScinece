...
#include "funcs.h"
	...
	//подсчет производной
	std::vector<float> scientificFuncs::paralelDerivate(std::vector<float> vecDataSet) {
	...
	std::vector<float> scientificFuncs::paralelSecDerivate(std::vector<float> vecDataSet) {
		cl::Program program = CreateProgram("mySecDerivateKernel.cl");
		cl::Context context = program.getInfo<CL_PROGRAM_CONTEXT>();
		std::vector<cl::Device> devices = context.getInfo<CL_CONTEXT_DEVICES>();
		auto& device = devices.front();
		...
		// выствляем аргументы Kernel
		cl::Kernel kernel(program, "calcSecDeriv_kernel");
		error_ret = kernel.setArg(0, dataBuf);
